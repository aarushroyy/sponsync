...
//   status: string;
//   updatedAt: string;
//   metricsProgress: MetricsProgressType[];
...
//   {(() => {
//     // Find the selected/accepted bundle or default to the first one
//     const selectedBundle = bundles.find(b => b.status === 'ACCEPTED') || bundles[0];
    
//     // Check if verifications exist
//     if (selectedBundle?.verifications && selectedBundle.verifications.length > 0) {
//       return (
//         <div className="space-y-6">
//           {selectedBundle.verifications.map((verification: VerificationType) => (
//             <Card key={verification.id}>
//               <CardHeader>
//                 <CardTitle>Verification by {verification.spocName}</CardTitle>
//                 <CardDescription>
//                   {verification.status === 'COMPLETED' ? 'Completed' : 'In Progress'} • 
//                   Last updated: {new Date(verification.updatedAt).toLocaleDateString()}
//                 </CardDescription>
//               </CardHeader>
//               <CardContent className="space-y-4">
//                 {/* Metrics Progress */}
...
//                   <div>
//                     <h3 className="font-medium mb-2">Metrics Verification:</h3>
//                     <div className="space-y-2">
//                       {verification.metricsProgress.map((progress: MetricsProgressType, index: number) => {
//                         // Find the target value from campaign metrics
//                         const campaignMetric = campaign.metrics.find(m => m.type === progress.type);
...
//                               <span className="text-sm">{progress.type.replace(/_/g, ' ')}</span>
...
//                   <div>
//                     <h3 className="font-medium mb-2">Verification Photos:</h3>
//                     <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
//                       {verification.verificationPhotos.map((photo: string, photoIndex: number) => (
//                         <div key={photoIndex} className="aspect-video rounded overflow-hidden">
//                           <img 
//                             src={photo} 
...
//                 {verification.report && (
//                   <div>
//                     <h3 className="font-medium mb-2">Verification Report:</h3>
...
//                 {/* If verification is not complete yet */}
//                 {verification.status !== 'COMPLETED' && (
//                   <Alert className="bg-yellow-50">
//                     <AlertDescription>
//                       This verification is still in progress. The SPOC is working on collecting all required information.
//                     </AlertDescription>
//                   </Alert>
//                 )}
//               </CardContent>
...
//           <p className="text-gray-500">No verifications available yet.</p>
//           <p className="text-sm text-gray-400 mt-2">
//             Verifications will appear here once a SPOC has been assigned to your bundle.
//           </p>
//         </div>
//       );
//     }
...
export default function CampaignDetailsPage() {
  ...
  // Add this log right before rendering the colleges
          ...
          <TabsContent value="bundle" className="space-y-6">
            {bundles.length === 0 ? (
              ...
              (() => {
                ...
                // Add this log right before rendering the colleges
console.log("College details:", selectedBundle.collegeDetails);
                  ...
                  <Card className="border-2 border-primary">
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div>
                        <h3 className="font-medium mb-2">Colleges:</h3>
                        <div className="space-y-2">
                          {selectedBundle.collegeDetails?.map((collegeDetail) => (
                            <div key={collegeDetail.id} className="flex justify-between items-center p-2 rounded bg-gray-50">
                              <div>
                                <div className="font-medium">{collegeDetail.college.collegeName}</div>
                                <div className="text-sm text-gray-600">
                                  {collegeDetail.college.eventName} ({collegeDetail.eventType})
                                </div>
                              </div>
                              <div className="flex flex-col items-end">
                                {collegeDetail.packageTier && (
                                  <Badge variant="outline">{collegeDetail.packageTier}</Badge>
                                )}
                                {collegeDetail.estimatedAmount && (
                                  <span className="text-sm font-semibold mt-1">
                                    ₹{collegeDetail.estimatedAmount.toLocaleString()}
                                  </span>
                                )}
                              </div>
                            </div>
