...
interface OnboardingFormData {
  ...
  poster: File | null;
  packageTier: PackageTier;
  totalBudgetGoal: string; // New field for total sponsorship goal
  packageConfigs: {
    [key in PackageTier]: PackageData;
  };
}
...
export default function CollegeOnboarding() {
  ...
  const initializePackageData = (tier: PackageTier): PackageData => {
    const config = PACKAGE_CONFIGS[tier];
    // Set default estimated amounts based on tier
    const defaultEstimatedAmount = tier === PackageTier.GOLD 
      ? "30000" 
    ...
    return {
      ...
      features: config.features,
      estimatedAmount: defaultEstimatedAmount,
    };
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-gray-100 py-12 px-4 sm:px-6 lg:px-8">
      <Card className="max-w-4xl mx-auto">
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-gray-900">College Onboarding</CardTitle>
          <CardDescription>Configure your event packages and sponsorship goals</CardDescription>
        </CardHeader>
